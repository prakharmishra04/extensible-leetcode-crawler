...
class TestFetchProblemUseCaseCacheHit:
    ...
    def test_execute_logs_cache_hit(
        self, use_case, mock_repository, mock_logger, sample_problem
    ):
        ...
        assert any(
            "Found problem 'two-sum' in cache" in str(call)
...
class TestFetchProblemUseCaseForceFlag:
    ...
    def test_execute_logs_force_flag(
        self, use_case, mock_client, mock_logger, sample_problem
    ):
        ...
        assert any(
            "Force flag set, bypassing cache" in str(call)
...
class TestFetchProblemUseCaseErrorHandling:
    ...
    def test_execute_raises_network_exception(
        self, use_case, mock_repository, mock_client
    ):
    ...
    def test_execute_logs_error_on_fetch_failure(
        ...
        assert any(
            "Failed to fetch problem 'two-sum'" in str(call)
    ...
    def test_execute_continues_on_repository_save_failure(
        ...
        """Test that execute returns problem even if repository save fails."""
        # Arrange
        mock_repository.find_by_id.return_value = None
        mock_client.fetch_problem.return_value = sample_problem
        ...
        # Assert - should still return the problem
        assert result is sample_problem
    
    def test_execute_logs_warning_on_repository_save_failure(
        self, use_case, mock_repository, mock_client, mock_logger, sample_problem
    ):
        """Test that execute logs warning when repository save fails."""
        # Arrange
        mock_repository.find_by_id.return_value = None
        mock_client.fetch_problem.return_value = sample_problem
        mock_repository.save.side_effect = RepositoryException("Disk full")
        ...
        assert any(
            "Failed to save problem 'two-sum' to repository" in str(call)
...
class TestFetchProblemUseCaseEdgeCases:
    ...
    def test_execute_with_empty_problem_id(
        self, use_case, mock_repository, mock_client
    ):
    ...
    def test_execute_with_empty_platform(
        self, use_case, mock_repository, mock_client
    ):
...
class TestFetchProblemUseCaseIntegration:
    ...
    def test_execute_full_flow_cache_miss_to_cache_hit(
        ...
        assert mock_repository.save.call_count == 1  # Still 1, not called again
