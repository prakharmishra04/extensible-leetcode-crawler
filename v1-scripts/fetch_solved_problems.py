...
def get_solved_by_status(client: LeetCodeClient) -> Dict[str, List[Dict]]:
    ...
    categorized = {"solved": [], "attempted": [], "not_started": []}
...
def save_to_file(
    problems: List[Dict], filename: str, format_type: str = "json", urls_only: bool = False
):
...
Examples:
  ...
  python fetch_solved_problems.py --details
    ...
    parser.add_argument(
        "--limit", type=int, default=100, help="Number of recent problems to fetch (default: 100)"
    )
    parser.add_argument(
        "--all-with-status",
        action="store_true",
    ...
    # Initialize client
    client = LeetCodeClient(session_cookie=args.session, csrf_token=args.csrf)
