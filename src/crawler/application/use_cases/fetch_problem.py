...
class FetchProblemUseCase:
    ...
    def execute(self, problem_id: str, platform: str, force: bool = False) -> Problem:
        ...
        if not force:
            self.logger.debug(
                f"Checking cache for problem '{problem_id}' on platform '{platform}'"
            )
            cached = self.repository.find_by_id(problem_id, platform)
            if cached:
                self.logger.info(
                    f"Found problem '{problem_id}' in cache for platform '{platform}'"
                )
                return cached
            self.logger.debug(
        ...
        # Fetch from platform
        self.logger.info(
            f"Fetching problem '{problem_id}' from platform '{platform}'"
        )
        try:
        ...
        try:
            self.logger.debug(
                f"Saving problem '{problem_id}' to repository for platform '{platform}'"
            )
            self.repository.save(problem)
            self.logger.debug(
                f"Successfully saved problem '{problem_id}' to repository"
            )
        except Exception as e:
            self.logger.warning(
                f"Failed to save problem '{problem_id}' to repository: {e}. "
                "Returning fetched problem anyway."
            )
            # Don't raise - we still have the problem from the API
