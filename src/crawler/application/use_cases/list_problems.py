...
class ListProblemsUseCase:
    ...
    def execute(self, options: ListOptions) -> List[Problem]:
        ...
        if options.difficulty:
            ...
            problems = [
                p for p in problems
                if p.difficulty.level in options.difficulty
            ]
            self.logger.debug(
        ...
        if options.topics:
            ...
            problems = [
                p for p in problems
                if any(topic in p.topics for topic in options.topics)
            ]
            self.logger.debug(
        ...
        # Sort problems
        self.logger.debug(
            f"Sorting problems by '{options.sort_by}' "
            f"(reverse={options.reverse})"
        )
        problems = self._sort_problems(problems, options.sort_by, options.reverse)
        
        self.logger.info(
            f"Returning {len(problems)} problems after filtering and sorting"
        )
        return problems
    
    def _sort_problems(
        self, problems: List[Problem], sort_by: str, reverse: bool
    ) -> List[Problem]:
        """
        ...
        try:
            ...
            self.logger.debug(
                f"Successfully sorted {len(sorted_problems)} problems "
                f"by '{sort_by}'"
            )
