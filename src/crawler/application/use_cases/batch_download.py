...
class BatchDownloadOptions:
    ...
    Example:
        ...
        >>> # Download only Easy and Medium problems on Array topic
        >>> options = BatchDownloadOptions(
        ...     username="john_doe",
        ...     platform="leetcode",
...
class BatchDownloadUseCase:
    ...
    def execute(self, options: BatchDownloadOptions) -> DownloadStats:
        ...
        try:
            self.logger.info(f"Fetching solved problems for user '{options.username}'")
            problems = self.client.fetch_solved_problems(options.username)
            self.logger.info(f"Found {len(problems)} solved problems")
        except Exception as e:
            self.logger.error(
                f"Failed to fetch solved problems for user '{options.username}': {e}"
            )
            raise
        ...
        for i, problem in enumerate(problems):
            try:
                ...
                self.logger.error(
                    f"Failed to download problem '{problem.id}': {e}",
                    exc_info=True
                )
    ...
    def _apply_filters(
        ...
        if options.difficulty_filter:
            self.logger.debug(
                f"Applying difficulty filter: {options.difficulty_filter}"
            )
            filtered = [
                p for p in filtered
                if p.difficulty.level in options.difficulty_filter
            ]
            self.logger.debug(
                f"After difficulty filter: {len(filtered)} problems remain"
            )
        ...
        if options.topic_filter:
            ...
            filtered = [
                p for p in filtered
                if any(topic in p.topics for topic in options.topic_filter)
            ]
    ...
    def _download_problem(
        ...
        if options.update_mode == UpdateMode.SKIP and problem_exists:
            self.logger.debug(
                f"Skipping problem '{problem.id}' (already exists, mode=SKIP)"
            )
            stats.skipped += 1
        ...
        if options.update_mode == UpdateMode.UPDATE and problem_exists:
            ...
            self.logger.debug(
                f"Skipping problem '{problem.id}' (already exists, mode=UPDATE)"
            )
            stats.skipped += 1
        ...
        try:
            self.logger.debug(
                f"Fetching submission for problem '{problem.id}' "
                f"by user '{options.username}'"
            )
    ...
    def _notify_start(self, total: int) -> None:
        ...
        for observer in self.observers:
            try:
                ...
                self.logger.warning(
                    f"Observer {observer.__class__.__name__} failed on_start: {e}"
                )
    ...
    def _notify_skip(self, problem: Problem, reason: str) -> None:
        ...
        for observer in self.observers:
            try:
                ...
                self.logger.warning(
                    f"Observer {observer.__class__.__name__} failed on_skip: {e}"
                )
    ...
    def _notify_error(self, problem: Problem, error: Exception) -> None:
        ...
        for observer in self.observers:
            try:
                ...
                self.logger.warning(
                    f"Observer {observer.__class__.__name__} failed on_error: {e}"
                )
